...
class TaskManagementTests(TestCase):
    ...
    def test_admin_access(self):
        ...
        # Set up user-admin relationship
        self.user.assigned_admin = self.admin
        self.user.save()
        
        # Create a task assigned to the user
        task = Task.objects.create(
            title="Admin Viewable Task",
            description="This task should be visible to the admin",
            assigned_to=self.user,
            due_date=date.today(),
            status="pending"
        )
        ...
        # Verify response and task visibility
        self.assertEqual(response.status_code, 200)
